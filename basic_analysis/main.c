...
int main(int argc, char const *argv[]){
    ...
    uint8_t origin_marker = 0;
    {
        ...
        marker_count += find_marker(&img_small, markers + 0,
            0, img_small.width / 2,
            0, img_small.height / 2);
        marker_count += find_marker(&img_small, markers + 1,
            img_small.width / 2, img_small.width,
            0, img_small.height / 2);
        marker_count += find_marker(&img_small, markers + 2,
            img_small.width / 2, img_small.width,
            img_small.height / 2, img_small.height);
        marker_count += find_marker(&img_small, markers + 3,
            0, img_small.width / 2,
            img_small.height / 2, img_small.height);
    ...
    error = lodepng_encode32_file("marker.png", img_small.buf, img_small.width, img_small.height);
    if(error) printf("error %u: %s\n", error, lodepng_error_text(error));
    ...
    error = lodepng_encode32_file("fields.png", img_small.buf, img_small.width, img_small.height);
    if(error) printf("error %u: %s\n", error, lodepng_error_text(error));
